...
class CombinedVisualizerRight(Node):
    def __init__(self):
        super().__init__('combined_visualizer_right')
        ...
        self.image_sub = self.create_subscription(
            Image, '/camera/rgb/right', self.image_callback, 10
        )
        self.segmentation_sub = self.create_subscription(
            SegmentationData, '/segmentation/data_right', self.segmentation_callback, 10
        )
        self.objects_sub = self.create_subscription(
            ObjectInfoArray, '/objects/fused_info_right', self.objects_callback, 10
        )
        
        # Publicador único
        self.viz_pub = self.create_publisher(Image, '/debug/camera_right', 10)
