...
class CombinedVisualizerLeft(Node):
    def __init__(self):
        super().__init__('combined_visualizer_left')
        ...
        self.image_sub = self.create_subscription(
            Image, '/camera/rgb/left', self.image_callback, 10
        )
        self.segmentation_sub = self.create_subscription(
            SegmentationData, '/segmentation/data_left', self.segmentation_callback, 10
        )
        self.objects_sub = self.create_subscription(
            ObjectInfoArray, '/objects/fused_info_left', self.objects_callback, 10
        )
        
        # Publicador único
        self.viz_pub = self.create_publisher(Image, '/debug/camera_left', 10)
