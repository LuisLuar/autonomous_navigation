...
class YOLOPCannyCandidatesLeft(Node):

    def __init__(self):
        super().__init__('yolop_canny_candidates_left')
        ...
        self.pub_pixels = self.create_publisher(
            PixelPoint,
            '/lane/pixel_candidates_left',
            10
        )

        #cv2.namedWindow('Lane Candidates Debug Left', cv2.WINDOW_NORMAL)
    ...
    def process(self):
        ...
        cv2.imshow('Lane Candidates Debug Left', vis)
