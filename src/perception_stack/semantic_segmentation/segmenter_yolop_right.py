...
class YOLOPv2SegmenterRight(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'Cámara derecha - Procesando cada {self.process_every_n_frames} imágenes (aprox. {self.target_fps:.1f} FPS)')
        ...
        self.get_logger().info(f'Cargando YOLOPv2 (cámara derecha) en {self.device}')
    ...
    async def process_image(self, msg: Image):
        ...
        try:
            ...
            self.get_logger().debug(f'Cámara derecha - Imagen procesada correctamente. Frame: {self.frame_counter}')

        except Exception as e:
            self.get_logger().error(f'Error en procesamiento (cámara derecha): {str(e)}')
        finally:
            self.is_processing = False
            # Verificar si hay una nueva imagen esperando mientras procesábamos
            if self.latest_msg is not None and self.latest_msg != msg:
                self.get_logger().debug('Cámara derecha - Procesando siguiente imagen acumulada')
