...
class YoloDetectorNode(Node):
    def __init__(self):
        ...
        self.pub_results = self.create_publisher(
            DetectionArray, '/detection/results', 1
        )

        # Nombres de clases
        self.class_names = self.model.names if hasattr(self.model, 'names') else {}
        #self.get_logger().info(f"Clases disponibles: {len(self.class_names)}")
        ...
        #- Tracking: {self.use_tracking} ({self.tracker_type})
        #- Confianza: {self.conf_thres}
        #- IOU: {self.iou_thres}
        #- Tamaño: {self.imgsz}
        #""")
