...
class PerceptionLeftRecorder(Node):
    def __init__(self):
        super().__init__('perception_left_recorder')
        ...
        self.node_name = "perception_left"
        ...
        # Subscripciones a temas de percepción de la cámara izquierda
        self.create_subscription(
            Image, '/camera/rgb/left',
            self.image_callback, 10)
        
        self.create_subscription(
            SegmentationData, '/segmentation/data_left',
            self.segmentation_callback, 10)
        
        self.create_subscription(
            ObjectInfoArray, '/objects/fused_info_left',
        ...
        #self.get_logger().info(f'PerceptionLeftRecorder inicializado')
    ...
    def logging_enabled_cb(self, msg):
        ...
        if msg.data != self.is_logging_enabled:
            ...
            if self.is_logging_enabled:
                #self.get_logger().info('Logging percepción izquierda HABILITADO')
                if self.current_log_path:
                    self._start_logging()
            else:
                #self.get_logger().info('Logging percepción izquierda DESHABILITADO')
                self._stop_logging()
    ...
    def image_callback(self, msg):
        """Recibe imagen RGB de cámara izquierda"""
        try:
    ...
    def segmentation_callback(self, msg):
        """Recibe datos de segmentación de cámara izquierda"""
        try:
            # Reconstruir máscara
    ...
    def objects_callback(self, msg):
        """Recibe objetos fusionados con información 3D de cámara izquierda"""
        try:
    ...
    def _start_logging(self):
        ...
        try:
            # Crear estructura de carpetas para cámara izquierda
            base_dir = Path(self.current_log_path)
            self.current_session_dir = base_dir / "perception_left"
            ...
            #self.get_logger().info(f'Logging percepción izquierda iniciado en: {self.current_session_dir}')
            ...
            if hasattr(self.current_objects, 'objects'):
                for obj in self.current_objects.objects:
                    obj_dict = {
                        ...
                        'bbox': [int(obj.u1), int(obj.v1), int(obj.u2), int(obj.v2)],
    ...
    def _stop_logging(self):
        ...
        if self.csv_file:
            try:
                self.csv_file.close()
                #self.get_logger().info(f'Datos de percepción izquierda guardados: {self.saved_frames} frames')
            except Exception as e:
                pass
    ...
    def _generate_session_summary(self):
        ...
        try:
            ...
            with open(summary_path, 'w') as f:
                f.write("=== RESUMEN DE SESIÓN DE PERCEPCIÓN IZQUIERDA ===\n\n")
                f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Nodo: {self.get_name()}\n")
                f.write(f"Cámara: LEFT\n")
